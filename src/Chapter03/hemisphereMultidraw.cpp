// original: hemisphereMultidraw.cpp by Sumanta Guha.
//
...
void fillIndices(void)
{
  int i, j;
  for (j = 0; j < q; j++)
  // Note: indexes for the jth index array (given in the loop below) correspond
  // exactly
  // to the points on the jth triangle strip in the drawing loop of
  // hemisphere.cpp.
  {
    for (i = 0; i <= p; i++) {
...
void drawScene(void)
{
  ...
  // Multidraw command equivalent to the drawing loop for(j = 0; j < q;
  // j++){...} in hemisphere.cpp.
